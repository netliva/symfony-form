...
class NetlivaTreeSelectType extends AbstractType
{
	...
	public function buildForm(FormBuilderInterface $builder, array $options): void
	{
		...
		$builder->addViewTransformer(new CallbackTransformer(
			...
			{
				if ($options["mapped"] and (is_array($data) or is_object($data)))
				{
	...
	private function prepareSelectedEntity ($entity, $opt)
	{
		$str = "";
		if (method_exists($entity, 'get' . ucfirst($opt['value'])))
		{
			if (method_exists($entity, 'get' . ucfirst($opt['where'])) and $entity->{'get' . ucfirst($opt['where'])}())
				$str .= $this->prepareSelectedEntity($entity->{'get' . ucfirst($opt['where'])}(), $opt);
			if ($str) $str .= " » ";
			$str .= $entity->{'get' . ucfirst($opt['value'])}();
		}
		return $str;
	}
	...
	{
		$view->vars['selected'] = null;
		$entities = $this->container->getParameter('netliva_form.treeselect_entities');
		$opt = $entities[$options['entity_alias']];
		if (
			!$options['multiselect']
			and array_key_exists('data', $view->vars)
			and is_object($view->vars['data'])
			and $view->vars['data'] instanceof $opt['class']
			and method_exists($view->vars['data'], 'get' . ucfirst($opt['where']))
			and method_exists($view->vars['data'], 'get' . ucfirst($opt['key']))
			and method_exists($view->vars['data'], 'get' . ucfirst($opt['value']))
		)
		{
			$view->vars['selected'] = $this->prepareSelectedEntity($view->vars['data'], $opt);
		}

		$view->vars['entity_options'] = $entities[$options['entity_alias']];
